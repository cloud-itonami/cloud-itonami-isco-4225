(ns inquiry.actor
  "InquiryDeskActor — the ISCO-08 4225 community inquiry actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one inquiry operation request
  ...
  The unconditional invariant: the InquiryDeskAdvisor can never
  directly commit a record the InquiryClerksGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [inquiry.advisor :as advisor]
            [inquiry.governor :as governor]
            [inquiry.ledger :as ledger]
            [inquiry.store :as store]))

(defn build-graph
  "Build a compiled InquiryDeskActor graph. `store` implements
  `inquiry.store/Store`. `advisor` implements
  `inquiry.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   ...
                   ;; ledger could not show that :publish-faq — external
                   ;; publication, which ALWAYS requires sign-off — had
                   ;; received it.
                   (fn [{:keys [request proposal disposition]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :kb-id (:kb-id proposal)
                                    :payload proposal}
